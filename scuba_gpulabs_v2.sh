...
print("[BIBLE] Loading teacher in NF4 (4x faster generation)...")
...
teacher.eval()
print("[BIBLE] Teacher loaded in NF4.")
...
print(f"[BIBLE] Generating domain expert samples (target: {TARGET_SAMPLES})...")
for domain, seeds in DOMAIN_SEEDS.items():
    if domain == "general":
        continue
    if len(samples) >= TARGET_SAMPLES * 0.75:
        print(f"[BIBLE] Domain cap reached at {len(samples)} samples, moving to general.")
        break
    ...
    for bs in range(0, len(pool), BATCH_SIZE):
        ...
        try:
            ...
            print(f"[BIBLE] Domain {domain} warning: {e}")
            continue
        elapsed = time.time() - start
        rate = len(samples) / elapsed * 60 if elapsed > 0 else 0
        eta = (TARGET_SAMPLES - len(samples)) / (len(samples) / elapsed) / 60 if len(samples) > 0 else 0
        print(f"[BIBLE] {domain}: {len(samples)}/{TARGET_SAMPLES} samples | {rate:.0f}/min | ETA {eta:.0f}min")
        with open(OUTPUT_PATH, "w") as f:
            for s in samples: f.write(json.dumps(s) + "\n")
    print(f"[BIBLE] Domain {domain} complete: {domain_count} samples")

# General samples to fill remaining target
print(f"[BIBLE] Generating general samples to fill to {TARGET_SAMPLES}...")
general_pool = []
for seed in DOMAIN_SEEDS["general"]:
    for v in VARIATIONS:
        general_pool.append(v.format(seed) if "{}" in v else seed)
remaining = max(0, TARGET_SAMPLES - len(samples) - (len(DPO_PAIRS) * 8))
...
for bs in range(0, len(general_pool), BATCH_SIZE):
    if len(samples) >= TARGET_SAMPLES - (len(DPO_PAIRS) * 8):
        break
    ...
    try:
        ...
        print(f"[BIBLE] General warning: {e}")
        continue
    elapsed = time.time() - start
    rate = len(samples) / elapsed * 60 if elapsed > 0 else 0
    eta = (TARGET_SAMPLES - len(samples)) / (len(samples) / elapsed) / 60 if len(samples) > 0 else 0
    print(f"[BIBLE] General: {len(samples)}/{TARGET_SAMPLES} | {rate:.0f}/min | ETA {eta:.0f}min")
    with open(OUTPUT_PATH, "w") as f:
        for s in samples: f.write(json.dumps(s) + "\n")
...
print("[DISTILL] Applying LoRA r=256...")
lora = LoraConfig(
...
print("[DISTILL] Starting distillation 5 epochs — this will take ~1-2 hours...")
trainer.train()
trainer.save_model(SFT_OUT)
tok.save_pretrained(SFT_OUT)
print(f"[DISTILL] Distillation complete -> {SFT_OUT}")
del teacher
torch.cuda.empty_cache()
...
print(f"[DPO] Starting DPO 3 epochs on {len(raw_dpo)} pairs...")
trainer.train()
trainer.save_model(DPO_OUT)
tok.save_pretrained(DPO_OUT)
print(f"[DPO] DPO complete -> {DPO_OUT}")

del model, ref, trainer
torch.cuda.empty_cache()
print("[DPO] Models unloaded, VRAM cleared.")
PYEOF
